...
void test_complete_fft(){
    ...
    for(unsigned l=2;l<MAX_PROC_FRAME_LENGTH_LOG2;l++){
        ...
        for(unsigned t=0;t<1<<LOOPS_LOG2;t++){
            ...
            int error = 0;
            int exp = sext(dsp_pseudo_rand_int32(&r), 5);
            for(unsigned i=0;i<proc_frame_length;i++){
                a[i].re = dsp_pseudo_rand_int32(&r)>>1;
                a[i].im = dsp_pseudo_rand_int32(&r)>>1;
                A[i].re = dsp_conv_int32_to_float(a[i].re, exp, &error);
                A[i].im = dsp_conv_int32_to_float(a[i].im, exp, &error);
                real[i] = A[i].re;
                imag[i] = A[i].im;
            }
            ...
            unsigned diff = dsp_abs_diff_vect_complex_int32(a, exp, A, proc_frame_length, &error);
            ...
            diff = dsp_abs_diff_vect_complex_int32(a, exp, A, proc_frame_length, &error);
            TEST_ASSERT_LESS_OR_EQUAL_UINT32_MESSAGE(l, diff, "Output delta is too large");
            TEST_ASSERT_FALSE_MESSAGE(error, "Conversion error");
        }
    }
    ...
    for(unsigned l=3;l<MAX_PROC_FRAME_LENGTH_LOG2;l++){
        ...
        for(unsigned t=0;t<1<<LOOPS_LOG2;t++){
            ...
            for(unsigned i=0;i<proc_frame_length;i++){
                ...
                A[i].re = dsp_conv_int32_to_float(a[i], exp, &error);
                A[i].im = 0.0;
                real[i] = A[i].re;
                imag[i] = A[i].im;
            }
...
//            dsp_fft_bit_reverse_and_forward_real(a, proc_frame_length, (const int32_t *)dsp_sine_lut[l-1], (const int32_t *)dsp_sine_lut[l]);
            exp += l;

            unsigned diff = dsp_abs_diff_vect_complex_int32((dsp_complex_int32_t *)a, exp, A, proc_frame_length/2, &error);
            TEST_ASSERT_LESS_OR_EQUAL_UINT32_MESSAGE(l, diff, "Output delta is too large");
            TEST_ASSERT_FALSE_MESSAGE(error, "Conversion error");
            ...
            diff = dsp_abs_diff_vect_complex_int32((dsp_complex_int32_t *)a, exp, A, proc_frame_length/2, &error);
            TEST_ASSERT_LESS_OR_EQUAL_UINT32_MESSAGE(l, diff, "Output delta is too large");
            TEST_ASSERT_FALSE_MESSAGE(error, "Conversion error");
        }
    }
