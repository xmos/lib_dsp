...
void test_bfp_vect_div32() {
    ...
    for(unsigned len=1;len<MAX_LEN;len++){
        ...
        for(unsigned itt=0;itt<ITT;itt++){
            ...
            int b_exp = -31 + (15 + (dsp_pseudo_rand_int32(&rand_seed)%6));
            int c_exp = -31 + (15 + (dsp_pseudo_rand_int32(&rand_seed)%6));

            unsigned b_hr = dsp_pseudo_rand_uint32(&rand_seed)%6 - c_exp;
            unsigned c_hr = dsp_pseudo_rand_uint32(&rand_seed)%6;
            for(unsigned i=0;i<len;i++){
                b[i] = dsp_pseudo_rand_int32(&rand_seed)>>b_hr;
                c[i] = dsp_pseudo_rand_int32(&rand_seed)>>c_hr;
            ...
            for (int i=0; i<len; i++) {
                A[i] = B[i] / C[i];
                printf("%f\n", A[i]);
                att_print_python_int32(&a[i], 1, a_exp);
                dsp_conv_float_to_int32(A[i], a_exp, &error);
                int c_cls = dsp_bfp_cls_int32(c[i]);
                int b_cls = dsp_bfp_cls_int32(b[i]);
                if ((b_cls + c_exp) + (31 - c_cls) < 0) {
                    printf("OVERFLOW!\n");
                }
                if (error) {

                    int m_exp;
                    double m = frexp (A[i], &m_exp);

                    double r = ldexp(m, m_exp - a_exp);
                    int output_exponent;
                    frexp(r, &output_exponent);

                    printf("######## Error\n");
                    printf("m_exp: %d\n", m_exp);
                    printf("output_exponent: %d\n", output_exponent);
                    printf("((%d + %d) + (32 - %d) = %d\n", b_cls, c_exp, c_cls,
                            (b_cls + c_exp) + (32 - c_cls));
            ...
            //printf("Error = %d\n", error);
            // Ignore conversion error: Division can get big!
            //TEST_ASSERT_FALSE_MESSAGE(error, "Conversion error");
            TEST_ASSERT_LESS_THAN_UINT32_MESSAGE(3, diff, "error is too high");
            TEST_ASSERT_EQUAL_UINT32_MESSAGE(a_hr, actual_a_hr, "error hr is wrong");
        }
