...
void txp_ramp(chanend x) {
    dsp_complex_t [[aligned(8)]] data[TRPCHANS][12];
    int cnt = 0;
    int advance = 12;
    for(int i = 0; i < 4; i++) {
        for(int j = 0; j < advance; j++) {
            ...
            data[1][j].re = (-cnt) << i;
            data[1][j].im = ((-cnt) ^ 5) << i;
            data[2][j].re = (cnt*cnt) << i;
            data[2][j].im = ((cnt*cnt)^9) << i;
        }
        dsp_bfp_tx_pairs(x, (data, dsp_complex_t[]), TRPCHANS, 12, i);
    }
}

void rxp_ramp(chanend x) {
    int advance = 12;
    int errors = 0;
    dsp_complex_t [[aligned(8)]] data[TRPCHANS][32];
    uint64_t state[DSP_BFP_RX_STATE_UINT64_SIZE(TRPCHANS, 32, 12)];
    dsp_bfp_rx_state_init_xc(state, DSP_BFP_RX_STATE_UINT64_SIZE(TRPCHANS, 32, 12));
    int start = advance - 32 + 1;// -19;
    for(int i = 0; i < 4; i++) {
        int shr = dsp_bfp_rx_pairs(x, state, (data, dsp_complex_t[]),
                                   TRPCHANS, 32, advance, 1);
        for(int j = 0; j < 32; j++) {
            int expected = start + j;
            if (expected < 0) expected = 0;
            int rxed = data[0][j].re >> shr;
            if (expected != rxed) {
                printf("Error: %2d not %2d (%d/%d) 0.r\n", rxed, expected, i, j);
                errors++;
            }
            rxed = data[0][j].im >> shr;
            if ((expected != 0 ? expected ^ 3 : expected) != rxed) {
                printf("Error: %2d not %2d (%d/%d) 0.i\n", rxed, expected, i, j);
                errors++;
            }
            rxed = data[1][j].re >> shr;
            if (-expected != rxed) {
                printf("Error: %2d not %2d (%d/%d) 1.r\n", rxed, -expected, i, j);
                errors++;
            }
            rxed = data[1][j].im >> shr;
            if ((expected != 0 ? -expected ^ 5 : expected) != rxed) {
                printf("Error: %2d not %2d (%d/%d) 1.i\n", rxed, -expected, i, j);
                errors++;
            }
            rxed = data[2][j].re >> shr;
            if (expected*expected != rxed) {
                printf("Error: %2d not %2d (%d/%d) 2.r\n", rxed, expected*expected, i, j);
                errors++;
            }
            rxed = data[2][j].im >> shr;
            if ((expected != 0 ? expected*expected ^ 9 : expected) != rxed) {
                printf("Error: %2d not %2d (%d/%d) 2.i\n", rxed, expected*expected, i, j);
                errors++;
            }
        }
        start += advance;
    ...
    if (errors == 0) {
        printf("dsp_bfp_rx/tx_p(%d) passed\n", advance);
    } else {
        printf("dsp_bfp_rx/tx_p(%d) FAIL with %d errors\n", advance, errors);
    }
