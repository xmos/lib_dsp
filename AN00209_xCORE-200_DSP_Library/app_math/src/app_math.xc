...
typedef enum { PV_OFF = 0, PV_ON = 1 } print_values_t;

typedef enum { PC_OFF = 0, PC_ON = 1 } print_cycles_t;

typedef enum { LINEAR = 0, EXPONENTIAL = 1 } stimulus_t;

/*
 * Report Errors within the Error Range. Top and Bottom or range contain
 * saturated values
 */
...
int32_t check_result(int32_t  result,
  ...
  if (error < e->max_negative_error)
    e->max_negative_error = error;
  if (error > e->max_positive_error)
    e->max_positive_error = error;
...
inline int32_t qs(double d, const int32_t q_format) {
  return (int32_t)((int64_t)((d) * ((uint64_t)1 << (q_format + 20)) + (1 << 19))
                   >> 20);
}
...
void test_multipliation_and_division() {
  ...
  /*
  The result of 0.0005 x 0.0005 is 0.00000025. But this number is not
  representable as a binary. The closest representation in Q8.24 format is
  (4/2^24) = 0.000000238418579 printf rounds this to 0.0000002 because the
  formatting string to printf specifies 8 digits of precision after the decimal
  point. This is the maximum precision that can be achieved with the 24
  fractional bits of the Q8.24 format.
  */
...
static void single_atan2_test(int      y,
  ...
  unsigned hypotlimit =
      (hypot >> (hypot_precision)) + ((hypot >> (hypot_precision - 1)) & 1) - 1;
...
void atan2_test(void) {
  ...
  for (int i = 0; i < 100; i += 1) {
    int x = get_random_number(); // Returns random number from INT_MIN to
                                 // INT_MAX
    int y = get_random_number();
    if (x < -0x7FFFFFFF)
      x = -0x7FFFFFFF; // Clip as arctan2 cannot take INT_MIN
    if (y < -0x7FFFFFFF)
      y = -0x7FFFFFFF;
  ...
  for (int i = 0; i < 65; i++) {
    for (int k = 0; k < 20; k++) {
      ...
      unsigned long long rrp =
          rr == 0xffffffff ? 0xffffffffffffffffull : (rr + 1ull) * (rr + 1ull);
      if (r <= rrm || r > rrp) {
        errors++;
