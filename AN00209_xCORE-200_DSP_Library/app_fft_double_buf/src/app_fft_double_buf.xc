...
#ifdef COMPLEX_FFT
typedef struct {
    lib_dsp_fft_complex_t sample_blocks[NUM_CHANS][N_FFT_POINTS];
} multichannel_sample_block_s ;

  #if N_FFT_POINTS*NUM_CHANS > 8192
  #error "Max N_FFT_POINTS*NUM_CHANS is 8192 for COMPLEX_FFT"
  #endif
#else
    ...
    select {
      case input.swap(multichannel_sample_block_s * movable &input_buf):
        ...
        for(int i=0; i<NUM_CHANS; i++) {
           lib_dsp_fft_bit_reverse( buffer->sample_blocks[i], N_FFT_POINTS );
           lib_dsp_fft_forward_complex(buffer->sample_blocks[i], N_FFT_POINTS, FFT_SINE_SHORT(N_FFT_POINTS) );
        }
#else
