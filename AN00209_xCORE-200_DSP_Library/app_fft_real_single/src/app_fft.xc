  ...
  unsafe {
    for (int N = 8; N <= MAXN; N *= 2) {
      ...
      for (int k = 1; k < 20; k++) {
        ...
        dsp_fft_bit_reverse_and_forward_real(
            real_single_data, N, (int32_t *) s(N / 2), (int32_t *) s(N));
        dsp_fft_bit_reverse(check, N);
        dsp_fft_forward(check, N, (int32_t *) s(N));

        check[0].im = check[N / 2].re;
        for (int i = 0; i < N; i += 2) {
          int abserr = abs(real_single_data[i] - check[i / 2].re) +
                       abs(real_single_data[i + 1] / (double) check[i / 2].im);
          if (abserr > 3) {
            printf("N: %d, k: %d Forward real err %d: %9d %9d %9d  %5.3f  "
                   "%5.3f %9d %9d\n",
                   N, k, abserr, copy[i], real_single_data[i],
                   real_single_data[i + 1],
                   real_single_data[i] / (double) check[i / 2].re,
                   real_single_data[i + 1] / (double) check[i / 2].im,
                   check[i / 2].re, check[i / 2].im);
            errors++;
          }
          maxferr = max(abserr, maxferr);
        }
        check[0].im = 0;
        dsp_fft_bit_reverse_and_inverse_real(
            real_single_data, N, (int32_t *) s(N / 2), (int32_t *) s(N));
        dsp_fft_bit_reverse(check, N);
        ...
        for (int i = 0; i < N; i++) {
          ...
          if (abserr > N) {
            printf(
                "N: %d, k: %d Inverse real err %d: %9d %9d  %5.3f  %9d %9d\n",
                N, k, abserr, copy[i], real_single_data[i],
                real_single_data[i] / (double) check[i].re, check[i].re,
                check[i].im);
            errors++;
          }
