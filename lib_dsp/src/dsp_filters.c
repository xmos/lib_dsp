...
// FIR filter (even coeff array boundary, no state data shifting - for internal
// use only)
...
// FIR filter (odd coeff array boundary, no state data shifting - for internal
// use only)
...
void dsp_filters_interpolate(int32_t       input,
  ...
  */
  ...
  for (int32_t i = 0; i < interp_factor; ++i) {
    if (odd)
      output_samples[i] =
          _dsp_filters_interpolate__fir_odd(coeff, state, length, q_format);
    else
      output_samples[i] =
          _dsp_filters_interpolate__fir_even(coeff, state, length, q_format);
    coeff += length;
    odd ^= length & 1;
  }
}
...
int32_t dsp_filters_decimate(int32_t       input_samples[],
  ...
  */
  ...
  for (int32_t i = 0; i < decim_factor - 1; ++i)
    state_data[i] = input_samples[i + 1];
  return output;
}
...
int32_t dsp_filters_biquad(int32_t        input_sample,
  ...
  asm("maccs %0,%1,%2,%3"
      : "=r"(ah), "=r"(al)
      : "r"(input_sample), "r"(c1), "0"(0), "1"(1 << (q_format - 1)));
  asm("ldd %0,%1,%2[0]" : "=r"(s2), "=r"(s1) : "r"(state_data));
