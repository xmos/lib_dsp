...
void dsp_matrix_adds(const int32_t *input_matrix_X, int32_t scalar_value_A,
  ...
  dsp_vector_adds(input_matrix_X, scalar_value_A, result_matrix_R,
...
void dsp_matrix_muls(const int32_t *input_matrix_X, int32_t scalar_value_A,
  ...
  dsp_vector_muls(input_matrix_X, scalar_value_A, result_matrix_R,
                  row_count * column_count, q_format);
}

void dsp_matrix_addm(const int32_t *input_matrix_X,
                     const int32_t *input_matrix_Y, int32_t *result_matrix_R,
                     int32_t row_count, int32_t column_count) {
  dsp_vector_addv(input_matrix_X, input_matrix_Y, result_matrix_R,
                  row_count * column_count);
}
...
void dsp_matrix_subm(const int32_t *input_matrix_X,
                     const int32_t *input_matrix_Y, int32_t *result_matrix_R,
                     int32_t row_count, int32_t column_count) {
  dsp_vector_subv(input_matrix_X, input_matrix_Y, result_matrix_R,
                  row_count * column_count);
}
...
void dsp_matrix_mulm(const int32_t *input_matrix_X,
  ...
  for (int32_t rx = 0; rx < rows_X; ++rx) {
    ...
    const int32_t *X_row_ptr =
        interface.get_array_ptr(0, rx); // matrix index and row vector index
#else
    ...
    for (int32_t cy = 0; cy < cols_Y; cy++) {
      // TODO: for large matrixes. provide the following arrays through shared
      // memory

#if MATRIX_Y_IN_EXTERNAL_RAM
      const int32_t *Y_column_ptr = interface.get_array_ptr(
          1, cy); // matrix index and column vector index
#else
...
void dsp_matrix_transpose(const int32_t *input_matrix_X,
  ...
  for (int32_t r = 0; r < row_count; ++r) {
    for (int32_t c = 0; c < column_count; ++c) {
      result_matrix_R[c * column_count + r] =
          input_matrix_X[r * column_count + c];
    }
  }
}
