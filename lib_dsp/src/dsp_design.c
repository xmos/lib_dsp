...
static int32_t _float2fixed(float x, int32_t q) {
  if (x < 0)
    return (((double)(1 << q)) * x - 0.5);
  else if (x > 0)
    return (((double)((1 << q) - 1)) * x + 0.5);
  return 0;
}

void dsp_design_biquad_notch(double frequency, double filter_Q,
                             int32_t coefficients[5], const int32_t q_format) {
...
void dsp_design_biquad_lowpass(double frequency, double filter_Q,
...
void dsp_design_biquad_highpass(double frequency, double filter_Q,
                                int32_t       coefficients[5],
                                const int32_t q_format) {
  // Compute common factors
  double w0    = 2.0 * pi * frequency;
...
void dsp_design_biquad_allpass(double frequency, double filter_Q,
                               int32_t       coefficients[5],
                               const int32_t q_format) {
  // Compute common factors
  double w0    = 2.0 * pi * frequency;
...
void dsp_design_biquad_bandpass(double frequency1, double frequency2,
                                int32_t       coefficients[5],
                                const int32_t q_format) {
  // Compute common factors
...
void dsp_design_biquad_peaking(double frequency, double filter_Q,
                               double gain_db, int32_t coefficients[5],
                               const int32_t q_format) {
  // Compute common factors
...
void dsp_design_biquad_lowshelf(double frequency, double filter_Q,
                                double shelf_gain_db, int32_t coefficients[5],
                                const int32_t q_format) {
  // Compute common factors
  double A  = pow(10.0, (shelf_gain_db / 40.0));
  double w0 = 2.0 * pi * frequency;
  double alpha =
      sin(w0) / 2 * sqrt((A + 1.0 / A) * (1.0 / filter_Q - 1.0) + 2.0);
...
void dsp_design_biquad_highshelf(double frequency, double filter_Q,
                                 double shelf_gain_db, int32_t coefficients[5],
                                 const int32_t q_format) {
  // Compute common factors
  double A  = pow(10.0, (shelf_gain_db / 40.0));
  double w0 = 2.0 * pi * frequency;
  double alpha =
      sin(w0) / 2.0 * sqrt((A + 1.0 / A) * (1.0 / filter_Q - 1.0) + 2.0);
