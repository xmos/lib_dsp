...
int32_t dsp_adaptive_lms(int32_t source_sample, int32_t reference_sample,
  ...
  output_sample = dsp_filters_fir(source_sample, filter_coeffs, state_data,
                                  num_taps, q_format);
  ...
  *error_sample = reference_sample - output_sample;
  ...
  dsp_vector_muls_addv(state_data, mu_err, (int32_t *)filter_coeffs,
                       (int32_t *)filter_coeffs, num_taps, q_format);
...
int32_t dsp_adaptive_nlms(int32_t source_sample, int32_t reference_sample,
  ...
  *error_sample = reference_sample - output_sample;
  ...
  for (qq = q_format, ee = energy; qq >= 0 && !(ee & 0x80000000); --qq)
    ee <<= 1;
  energy = energy >> (q_format - qq);
  // Saturate the reciprocal value to max value for the given q_format
  if (energy < (1 << (31 - (31 - qq) * 2)))
    energy = (1 << (31 - (31 - qq) * 2)) + 0;
  ...
  dsp_vector_muls_addv(state_data, adjustment, (int32_t *)filter_coeffs,
                       (int32_t *)filter_coeffs, num_taps, q_format);
