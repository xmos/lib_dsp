...
// coefficients are scaled up for improved rounding
// they are also changed to positive values to enable using the dedicated
// instruction fur uint32_t long division:
...
/*
 * This function can be further improved by making EXP_C2 a constant in 3.29
 * format. The current function will have an error in the 21st or 22nd bit of
 * the fraction after a modulo of a large number. However, this would require
 * mulf3_29 functions adding code size. I think it is better to stick to a
 * slightly less accurate version for now.
 */
q8_24 dsp_math_exp(q8_24 x) {
  ...
  // q_format = input1 fraction bit count + input2 fraction bit count - result
