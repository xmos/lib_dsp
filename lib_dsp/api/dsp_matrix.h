 ...
 *  int32_t input_scalar_A = Q28( 0.333 );
 *  int32_t result_vector_R[8][32];
 *  dsp_matrix_muls( input_matrix_X, scalar_value_A, result_matrix_R, 256, 8,
 * 32, 28 ); \endcode
 *
 ...
 *  multiplication and q-format adjustment overflow behavior must be considered
 *  (see behavior for the function ``dsp_math_multiply``).
 *  The algorithm is optimised for performance using double word load and store
 * instructions. As a result the matrices must have an even number of rows and
 * columns.
 *
 *  Example:
 *  MxN * NxP = MxP
 ...
 *  int32_t input_matrix_Y[columns_Y][N]; // transposed for better memory
 * alighment !! int32_t result_matrix_R[rows_x][columns_Y]; dsp_matrix_mulm(
 * input_matrix_X, input_matrix_Y, result_matrix_R, 256, 8, 32, 28 ); \endcode
 *
 *  \param  input_matrix_X   Pointer to source data array X.
 *  \param  input_matrix_Y   Pointer to source data array Y.
 *  \param  result_matrix_R  Pointer to the resulting 2-dimensional data array.
 *  \param  rows_X           Number of rows in input matrix X. Must be even or
 * will trap. \param  cols_Y           Number of columns input matrix Y. Must be
 * even or will trap. \param  cols_X_rows_Y    Number of columns in input matrix
 * X == rows in input matrix Y. Must be even or will trap. \param  q_format
