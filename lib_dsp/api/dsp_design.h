 ...
 * ``b0,b1,b2,-a1,-a2``). The frequency specification is normalized to the
 * Nyquist frequency therefore the frequency value must be in the range of ``0.0
 * <= F < 0.5`` for valid filter coefficients.
 *
 *  Example showing a filter coefficients generation using Q28 fixed-point
 * formatting.
 *
 *  \code
 *  int32_t coeffs[5];
 ...
 *  \endcode
 *
 *  \param  filter_frequency   Filter center frequency normalized to the
 * sampling frequency.
 *                             ``0 < frequency < 0.5``, where 0.5 represents
 * Fs/2. \param  filter_Q           The filter Q-factor. \param  biquad_coeffs
 * The array used to contain the resulting filter coefficients. Filter
 * coefficients are ordered as ``[b0,b1,b2,-a1,-a2]``. \param  q_format Fixed
 ...
 *  \endcode
 *
 *  \param  filter_frequency   Filter cutoff (-3db) frequency normalized to the
 * sampling frequency.
 *                             ``0 < frequency < 0.5``, where 0.5 represents
 * Fs/2. \param  filter_Q           The filter Q-factor. \param  biquad_coeffs
 * The array used to contain the resulting filter coefficients. Filter
 ...
 *  \endcode
 *
 *  \param  filter_frequency1  Filter cutoff #1 (-3db) frequency normalized to
 * the sampling frequency.
 *                             ``0 < frequency1 < 0.5``, where 0.5 represents
 * Fs/2. \param  filter_frequency2  Filter cutoff #2 (-3db) frequency normalized
 * to the sampling frequency.
 *                             ``0 < frequency2 < 0.5``, where 0.5 represents
 * Fs/2. Note that frequency1 must be less than to frequency2. \param
 * biquad_coeffs      The array used to contain the resulting filter
 * coefficients. Filter coefficients are ordered as ``[b0,b1,b2,-a1,-a2]``.
 *  \param  q_format           Fixed point format of coefficients (i.e. number
 * of fractional bits).
 */
 ...
 * sampling frequency.
 *                             ``0 < frequency < 0.5``, where 0.5 represents
 * Fs/2. \param  filter_Q           The filter Q-factor. \param  peak_qain_db
 * The filter gain in dB (postive or negative). +gain results in peaking gain
 * (gain at peak center = ``gain_db``). -gain results in attenuation (gain at
 * peak center = -``gain_db``). \param  biquad_coeffs      The array used to
 * contain the resulting filter coefficients. Filter coefficients are ordered as
 * ``[b0,b1,b2,-a1,-a2]``. \param  q_format           Fixed point format of
 ...
 *  \endcode
 *
 *  \param  filter_frequency   Filter frequency (+3db or -3db point) normalized
 * to Fs.
 *                             ``0 < frequency < 0.5``, where 0.5 represents
 * Fs/2. \param  filter_Q           The filter Q-factor. \param  shelf_gain_db
 * The filter shelf gain in dB (postive or negative). +gain results in bass
 * shelf with gain of 'shelf_gain_db'. -gain results in bass shelf with
 * attenuation of 'shelf_gain_db'. \param  biquad_coeffs      The array used to
 * contain the resulting filter coefficients. Filter coefficients are ordered as
 * ``[b0,b1,b2,-a1,-a2]``. \param  q_format           Fixed point format of
 * coefficients (i.e. number of fractional bits).
 */
