 ...
 *
 *  This function sets each element of the result vector to the absolute value
 * of the corresponding input vector element.
 *
 *  Example:
 *
 *  \code
 ...
 *
 *  The elements in vector X are multiplied with the scalar A and stored in
 * result vector R. Each multiplication produces a 64-bit result. If overflow
 * occurs it is saturated at the minimum/maximum value given the fixed-point
 * format and finally shifted right by ``q_format`` bits.
 *
 *  Example:
 *
 *  \code
 ...
 *  \param  result_vector_R   Pointer to the resulting data array.
 *  \param  vector_length     Length of the input and output vectors.
 *  \param  q_format          Fixed point format, the number of bits making up
 * fractional part.
 */
 ...
 *  If overflow occurs it is saturated at the minimum/maximum value given the
 * fixed-point format and finally shifted right by ``q_format`` bits.
 *
 *  Example:
 *
 *  \code
 ...
 *  \param  result_vector_R   Pointer to the resulting data array.
 *  \param  vector_length     Length of the input and output vectors.
 *  \param  q_format          Fixed point format (i.e. number of fractional
 * bits).
 */
 ...
 *  Each multiplication produces a 64-bit result.
 *  If overflow occurs it is saturated at the minimum/maximum value given the
 * fixed-point format and finally shifted right by ``q_format`` bits. Then the
 * scalar is added. Overflow may occur after the addition.
 *
 *  Example:
 *
 *  \code
 ...
 *  int32_t input_scalar_A = Q28( 0.333 );
 *  int32_t result_vector_R[256];
 *  dsp_vector_mulv_adds( input_vector_X, input_vector_Y, scalar_value_A,
 * result_vector_R, 256, 28 ); \endcode
 *
 *  \param  input_vector_X    Pointer to source data array X.
 *  \param  input_vector_Y    Pointer to source data array Y.
 ...
 *  Each multiplication produces a 64-bit result.
 *  If overflow occurs it is saturated at the minimum/maximum value given the
 * fixed-point format and finally shifted right by ``q_format`` bits. Overflow
 * may occur after the addition Y[i].
 *
 *  Example:
 *
 *  \code
 ...
 *  int32_t input_vector_Y[256] = { 0, 1, 2, 3, ... not shown for brevity };
 *  int32_t result_vector_R[256];
 *  dsp_vector_muls_addv( input_vector_X, input_scalar_A, input_vector_Y,
 * result_vector_R, 256, 28 ); \endcode
 *
 *  \param  input_vector_X    Pointer to source data array X.
 *  \param  input_scalar_A    Scalar value to multiply each element by.
 ...
 *  If overflow occurs it is saturated at the minimum/maximum value given the
 * fixed-point format and finally shifted right by ``q_format`` bits. Overflow
 * may occur after the subtraction of Y[i].
 *
 *  Example:
 *
 *  \code
 ...
 *  int32_t result_vector_R[256];
 *  dsp_vector_muls_subv( input_vector_X, input_scalar_A, input_vector_Y,
 * result_vector_R, 256, 28 ); \endcode
 *
 *  \param  input_scalar_A    Scalar value to multiply each element by.
 *  \param  input_vector_X    Pointer to source data array X.
 ...
 *  If overflow occurs it is saturated at the minimum/maximum value given the
 * fixed-point format and finally shifted right by ``q_format`` bits. Overflow
 * may occur after the addition of Z[i].
 *
 *  Example:
 *
 *  \code
 ...
 *  If overflow occurs it is saturated at the minimum/maximum value given the
 * fixed-point format and finally shifted right by ``q_format`` bits. Overflow
 * may occur after the subtraction of Z[i].
 *
 *  Example:
 *
 *  \code
 ...
 *
 *  Vectors X[i] and Y[i] are complex, with separate arrays for the real and
 ...
 *  dsp_vector_mulv_complex( input_vector_X_re, input_vector_X_im,
 * input_vector_Y_re, input_vector_Y_im, result_vector_R_re, result_vector_R_im,
 * 256, 28 ); \endcode
 *
