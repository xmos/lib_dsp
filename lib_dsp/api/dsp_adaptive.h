 ...
 *  and 64-bit accumulation as a result of using a FIR filter as well as
 * coefficient step size calculations.
 *
 *  Multiplication results are accumulated in a 64-bit accumulator with the
 * final result shifted to the required fixed-point format. Therefore overflow
 ...
 *  \param  error_sample      Pointer to resulting error sample (error =
 * reference - output) \param  filter_coeffs     Pointer to FIR coefficients
 * arranged as [b0,b1,b2, ...,bN-1]. \param  state_data        Pointer to FIR
 * filter state data array of length ``N``. Must be initialized at startup to
 * all zeros. \param  num_taps          Filter tap count where ``N`` =
 * ``num_taps`` = filter order + 1. \param  mu                Coefficient
 * adjustment step size, controls rate of convergence. \param  q_format Fixed
 * point format (i.e. number of fractional bits). \returns                  The
 ...
 * the a transfer function that minimizes the error between the input and
 * reference signals. FIR coefficients are adjusted on a per sample basis by an
 * amount calculated from the given step size and the instantaneous error.
 *
 *  The function operates on a single sample of input and output data (i.e. and
 * each call to the function processes one sample and each call results in
 * changes to the FIR coefficients).
 ...
 * reference - output) \param  filter_coeffs     Pointer to FIR coefficients
 * arranged as [b0,b1,b2, ...,bN-1]. \param  state_data        Pointer to FIR
 * filter state data array of length N. Must be initialized at startup to all
 * zeros. \param  num_taps          Filter tap count where N = num_taps = filter
 * order + 1. \param  mu                Coefficient adjustment step size,
 * controls rate of convergence. \param  q_format          Fixed point format
 * (i.e. number of fractional bits). \returns                  The resulting
